...
Verify cuda
```
conda list -n textgen
```

#### 3. Install the web UI
Create a ```oobabooga``` folder
```
mkdir D:\aigc\oobabooga
D:
cd D:\aigc\oobabooga
```
```
...
set ENVNAME=textgen
rem Use conda info command to find out active env location
...
Download the webui.py from https://github.com/oobabooga/one-click-installers/blob/main/webui.py and save it to the parent folder of text-generation-webui

Replace ```conda_env_path = os.path.join(script_dir, "installer_files", "env")``` to
```
conda_env_path = "C:\\Users\\aigc\\.conda\\envs\\textgen"
```
Replace ``` conda_bat_path = os.path.join(script_dir, "installer_files", "conda", "condabin", "conda.bat")``` to your conda.bat path like:
```
conda_bat_path = "C:\\miniconda3\\condabin\\conda.bat"
```

Replace CMD_FLAGS like:
```
CMD_FLAGS = '--chat --api --listen --listen-host 192.168.0.21 --extensions character_bias'
```
